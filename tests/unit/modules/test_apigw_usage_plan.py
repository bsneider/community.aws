...
from plugins.modules import apigw_usage_plan
from plugins.modules.apigw_usage_plan import ApiGwUsagePlan
...
class TestApiGwUsagePlan(unittest.TestCase):
    ...
    def test_process_request_calls_get_usage_plans_and_stores_result_when_invoked(self, m):
        ...
        self.usage_plan.client.get_usage_plans = mock.MagicMock(
            return_value=resp)
    ...
    def test_process_request_calls_exit_json_with_expected_value_after_successful_delete(self, mr, md):
        ...
        self.usage_plan.module.exit_json.assert_called_once_with(
            changed='Egah!', usage_plan=None)
    ...
    def test_process_request_removes_api_stages_before_executing_delete(self, m):
        ...
        self.assertEqual(len(expected_patches), len(
            self.usage_plan.client.update_usage_plan.call_args[1]['patchOperations']))
        self.assertItemsEqual(
            expected_patches, self.usage_plan.client.update_usage_plan.call_args[1]['patchOperations'])
    ...
    def test_process_request_calls_delete_usage_plan_when_state_absent_and_usage_plan_found(self, m):
        ...
        self.usage_plan.client.delete_usage_plan.assert_called_once_with(
            usagePlanId='found')
    ...
    def test_process_request_calls_fail_json_when_delete_usage_plan_raises_error(self, m):
        ...
        self.usage_plan.client.delete_usage_plan = mock.MagicMock(
            side_effect=BotoCoreError)
        self.usage_plan.process_request()
    ...
    def test_process_request_calls_exit_json_with_expected_value_after_successful_create(self, mra, mca):
        ...
        self.usage_plan.module.exit_json.assert_called_once_with(
            changed='eye', usage_plan='on the sandwich')
    ...
    @patch('plugins.modules.apigw_usage_plan.ApiGwUsagePlan._create_patches', return_value=[])
    ...
    def test_process_request_skips_update_usage_plan_and_replies_false_when_no_changes(self, m, mcp):
        ...
        self.assertEqual(
            0, self.usage_plan.client.update_usage_plan.call_count)
        self.usage_plan.module.exit_json.assert_called_once_with(
            changed=False, usage_plan={'something': 'here'})

    @patch('plugins.modules.apigw_usage_plan.ApiGwUsagePlan._create_patches', return_value=['patches!'])
    @patch.object(ApiGwUsagePlan, '_retrieve_usage_plan', return_value={'id': 'hi'})
    ...
    def test_process_request_returns_result_of_find_when_update_is_successful(self, m, mcp):
        ...
        self.usage_plan.module.exit_json.assert_called_once_with(
            changed=True, usage_plan='second call')
    ...
    def test_process_request_skips_update_usage_plan_and_replies_true_when_check_mode(self, m, mcp):
        ...
        self.assertEqual(
            0, self.usage_plan.client.update_usage_plan.call_count)
        self.usage_plan.module.exit_json.assert_called_once_with(
            changed=True, usage_plan={'something': 'here'})
    ...
    def test_define_argument_spec(self):
        ...
        self.assertEqual(result, dict(
                         ...
                         throttle_burst_limit=dict(
                             required=False, default=-1, type='int'),
                         throttle_rate_limit=dict(
                             required=False, default=-1.0, type='float'),
                         quota_limit=dict(
                             required=False, default=-1, type='int'),
                         quota_offset=dict(
                             required=False, default=-1, type='int'),
                         quota_period=dict(required=False, default='', choices=[
                                           '', 'DAY', 'WEEK', 'MONTH']),
    ...
    def test_main(self, mock_ApiGwUsagePlan, mock_AnsibleModule):
        ...
        mock_ApiGwUsagePlan.assert_called_once_with(
