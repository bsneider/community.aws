...
class TestApiGwModel(unittest.TestCase):
    ...
    def test_process_request_calls_exit_json_with_changed_and_response_from_create_model(self, mockFindModel, mockCreateModel):
        ...
        self.module.exit_json.assert_called_with(
            changed=mock_changed, model=mock_response)
    ...
    def test_create_model_calls_fail_json_if_create_model_throws_exception(self):
        self.model.client.create_model.side_effect = ClientError(
            {'Error': {'Code': 'x NotFoundException x'}}, 'error')

        self.model._create_model()

        self.module.fail_json.assert_called_with(
            msg='Error while creating model: An error occurred (x NotFoundException x) when calling the error operation: Unknown')
    ...
    def test_update_model_calls_fail_json_if_update_model_throws_exception(self):
        self.model.client.update_model.side_effect = ClientError(
            {'Error': {'Code': 'some error'}}, 'error')

        self.model._update_model()

        self.module.fail_json.assert_called_with(
            msg='Error while updating model: An error occurred (some error) when calling the error operation: Unknown')
    ...
    def test_delete_model_returns_nothing_if_delete_model_returns_not_found_exception(self):
        ...
        self.model.client.delete_model.side_effect = ClientError(
            {'Error': {'Code': '(NotFoundException)'}}, 'error')
    ...
    def test_delete_model_calls_fail_json_if_delete_model_returns_any_other_exception(self):
        self.model.client.delete_model.side_effect = ClientError(
            {'Error': {'Code': '(AnyOtherException)'}}, 'error')

        self.model._delete_model()

        self.module.fail_json.assert_called_with(
            msg='Error while deleting model: An error occurred ((AnyOtherException)) when calling the error operation: Unknown')
    ...
    def test_find_model_returns_None_if_model_does_not_exist(self):
        self.model.client.get_model.side_effect = ClientError(
            {'Error': {'Code': 'x NotFoundException x'}}, 'something')
