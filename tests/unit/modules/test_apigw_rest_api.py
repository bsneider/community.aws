...
from plugins.modules import apigw_rest_api
from plugins.modules.apigw_rest_api import ApiGwRestApi
...
class TestApiGwRestApi(unittest.TestCase):
    ...
    def test_process_request_exits_with_no_change_when_removing_non_existent_api(self):
        ...
        self.restapi.module.exit_json.assert_called_once_with(
            changed=False, api=None)
    ...
    def test_process_request_exits_with_no_change_when_adding_existing_and_unchanged_api(self):
        ...
        self.restapi.module.params = {
            'name': 'whatever', 'state': 'present', 'description': 'very awesome'}
        self.restapi.client.get_rest_apis = mock.MagicMock(
            return_value=get_response)
        self.restapi.process_request()

        self.restapi.module.exit_json.assert_called_once_with(
            changed=False, api=get_response['items'][0])
    ...
    def test_process_request_creates_api_when_missing(self):
        ...
        self.restapi.client.create_rest_api.assert_called_once_with(
            name='whatever', description='very awesome')
        self.restapi.module.exit_json.assert_called_once_with(
            changed=True, api=create_response)
    ...
    def test_process_request_updates_api_when_params_do_not_match(self):
        ...
        self.restapi.module.params = {
            'name': 'whatever', 'state': 'present', 'description': 'awesomer'}
        self.restapi.client.get_rest_apis = mock.MagicMock(
            return_value=get_response)
        self.restapi.client.update_rest_api = mock.MagicMock(
            return_value='TotallyUpdated')
        self.restapi.process_request()
        ...
        self.restapi.module.exit_json.assert_called_once_with(
            changed=True, api='TotallyUpdated')
