...
# Find tests
    ...
    def test_process_request_calls_fail_json_when_ClientError_is_not_NotFoundException(self, mock_create):
        ...
        self.method.module.fail_json.assert_called_once_with(
            msg='Error calling boto3 get_method: An error occurred (boom) when calling the xxx operation: error')
    ...
    def test_process_request_calls_fail_json_when_get_method_throws_other_boto_core_error(self, mock_create):
        ...
        self.method.module.fail_json.assert_called_once_with(
            msg='Error calling boto3 get_method: An unspecified error occurred')
# End find teste
...
# Delete tests
    ...
    def test_process_request_deletes_method_when_method_is_present(self, mock_find):
        ...
        self.method.module.exit_json.assert_called_once_with(
            changed=True, method=None)
...
# Update
    ...
    def test_process_request_calls_update_method_when_present_and_changed(self, mock_find, mock_vp):
        ...
        expected_patch_ops = [
            ...
            {'op': 'add', 'path': '/requestParameters/method.request.path.bob',
                'value': 'True'},
            ...
            {'op': 'replace', 'path': '/requestParameters/method.request.header.frank',
                'value': 'False'},
        ]
    ...
    def test_process_request_calls_update_method_with_patch_operations_to_add_update_or_remove_request_models_for_method(self, mock_find, mock_vp):
        ...
        expected_patch_ops = [
            ...
            {'op': 'replace', 'path': '/requestModels/application~1json',
                'value': 'new_value'},
            {'op': 'add', 'path': '/requestModels/application~1pdf', 'value': 'pdf'}
        ]
    ...
    def test_process_request_skips_update_and_returns_true_when_check_mode_set(self, mock_find, mock_vp):
        ...
        self.assertEqual(
            0, self.method.client.update_method_response.call_count)
        self.assertEqual(
            0, self.method.client.delete_method_response.call_count)
        self.assertEqual(
            0, self.method.client.put_integration_response.call_count)
        self.assertEqual(
            0, self.method.client.update_integration_response.call_count)
        self.assertEqual(
            0, self.method.client.delete_integration_response.call_count)
        self.method.module.exit_json(changed=True, method=None)

    @patch('plugins.modules.apigw_method.update_method', mock_args)
    @patch.object(ApiGwMethod, 'validate_params')
    @patch.object(ApiGwMethod, '_find_method', return_value={})
    def test_process_request_calls_fail_json_when_update_method_raises_exception(self, mock_find, mock_vp):
        self.method.client.update_method = mock.MagicMock(
            side_effect=BotoCoreError())
        self.method.process_request()
    ...
    def test_process_request_calls_update_integration_when_present_and_changed(self, mock_find, mock_vp):
        ...
        expected_patch_ops = [
            ...
            {'op': 'replace', 'path': '/requestTemplates/change~1me',
                'value': 'changeval'},
        ]
    ...
    def test_process_request_adds_integration_type_and_http_method_for_certain_integration_types(self, mock_find, mock_vp):
        ...
        for t in ['AWS', 'HTTP', 'AWS_PROXY']:
            ...
            self.assertEqual(
                'a majestic uri', self.method.client.put_integration.call_args[1]['uri'])
            self.assertEqual(
                'methody method', self.method.client.put_integration.call_args[1]['integrationHttpMethod'])
    ...
    def test_process_request_calls_fail_json_when_update_integration_raises_exception(self, mock_find, mock_vp):
        self.method.client.update_integration = mock.MagicMock(
            side_effect=BotoCoreError())
        self.method.process_request()
    ...
    def test_process_request_updates_method_responses_when_present_and_changed(self, mock_find, mock_vp):
        ...
        expected_patch_ops = [
            ...
            {'op': 'add', 'path': '/responseParameters/method.response.header.addparam',
                'value': 'False'},
    ...
    def test_process_request_adds_models_and_params_when_missing_from_aws(self, mock_find, mock_vp):
        mock_find.return_value = {
            'methodResponses': {'202': {'statusCode': '202'}}}
    ...
    def test_process_request_updates_method_integrations_when_present_and_changed(self, mock_find, mock_vp):
        ...
        expected_patch_ops = [
            {'op': 'replace', 'path': '/selectionPattern',
                'value': 'totally-invalid-but-whatever'},
            {'op': 'replace', 'path': '/responseTemplates/application~1json',
                'value': 'new-value'},
            {'op': 'add', 'path': '/responseTemplates/add', 'value': 'me'},
            {'op': 'remove', 'path': '/responseTemplates/delete'},
            {'op': 'replace', 'path': '/responseParameters/method.response.header.change',
                'value': 'newval'},
            {'op': 'add', 'path': '/responseParameters/method.response.body.addme',
                'value': 'bodyval'},
    ...
    def test_process_request_calls_get_method_and_returns_result_after_update_when_method_is_up_to_date(self, mock_find):
        ...
        self.method.module.exit_json.assert_called_once_with(
            changed=False, method='Called post-update')
...
# Create tests
    ...
    def test_process_request_calls_get_method_and_returns_result_after_create_when_method_is_absent(self, mock_find):
        ...
        self.method.module.exit_json.assert_called_once_with(
            changed=True, method='Called post-create')

    @patch.object(ApiGwMethod, '_find_method', return_value=None)
    def test_process_request_calls_put_method_when_method_is_absent(self, mock_find):
        self.method.module.params['request_models'] = [
            {'content_type': 'application/json', 'model': 'ModelName'}]
    ...
    def test_process_request_calls_put_method_response_when_method_is_absent(self, mock_find):
        p = [
            {'status_code': 200, 'response_models': [
                {'content_type': 'ct1', 'model': 'model'}, {'content_type': 'ct2'}]},
            {'status_code': 400, 'response_params': [
                {'name': 'err_param', 'is_required': True}]},
            {'status_code': 500}
        ]
    ...
    def test_process_request_calls_put_integration_response_when_method_is_absent(self, mock_find):
        p = [
            ...
            },
            {
                ...
                'pattern': '.*Bad Request.*',
                'response_params': [
                    {'name': 'body_param', 'value': 'json-expression',
                        'location': 'body'},
                    {'name': 'header_param', 'value': 'headerval', 'location': 'header'}
                ],
        ...
        self.assertEqual(
            2, self.method.client.put_integration_response.call_count)
        for kwargs in expected:
            self.method.client.put_integration_response.assert_any_call(
                **kwargs)
    ...
    def test_process_request_calls_fail_json_when_put_integration_response_throws_error(self, mock_find):
        self.method.client.put_integration_response = mock.MagicMock(
            side_effect=BotoCoreError())
        self.method.module.params['integration_responses'] = [
    ...
    def test_validation_of_arguments(self):
        ...
        for test in tests:
            ...
            try:
                ...
                self.assertEqual("Error validating {0}: {1}".format(
                    test['error']['param'], test['error']['msg']), str(e))
...
# End validation
    ...
    def test_define_argument_spec(self):
        ...
        self.assertEqual(result, dict(
                         ...
                         method_integration=dict(
                             type='dict',
                             default={},
                             integration_type=dict(required=False, default='AWS', choices=[
                                                   'AWS', 'MOCK', 'HTTP', 'HTTP_PROXY', 'AWS_PROXY']),
                             http_method=dict(required=False, default='POST', choices=[
                                              'POST', 'GET', 'PUT']),
                             uri=dict(required=False),
                             credentials=dict(required=False),
                             passthrough_behavior=dict(required=False, default='when_no_templates', choices=[
                                                       'when_no_templates', 'when_no_match', 'never']),
                         ...
                         method_responses=dict(
                             ...
                             response_models=dict(
                                 ...
                                 default=[],
                                 content_type=dict(required=True),
                                 model=dict(required=False, default='Empty', choices=[
                                     'Empty', 'Error'])
                             )
                         ),
    ...
    def test_main(self, mock_ApiGwMethod, mock_AnsibleModule):
        ...
        self.assertEqual(
            1, mock_ApiGwMethod_instance.process_request.call_count)
